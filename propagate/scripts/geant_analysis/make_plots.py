...
c.SaveAs(os.path.join(outdir, "rate_comps","mcTruth_atLeast1PE.png"))
c.SaveAs(os.path.join(outdir, "rate_comps","mcTruth_atLeast1PE.pdf"))


## nPE histograms
mkdir(os.path.join(outdir, "nPE"))
mqs = [(0.5,0.01),(0.5,0.03),(0.5,0.05),(0.5,0.07),(0.5,0.1)]
...
hdummy = r.TH1F("hdummy5","", 100, 3e-3, 2e0)
hdummy.SetLineColor(r.kWhite)
hdummy.GetXaxis().SetRangeUser(3e-3,2e0)
# hdummy.GetYaxis().SetRangeUser(1e-4, 1e1)
hdummy.GetYaxis().SetRangeUser(1e-2, 3e0)
...
gs = []
leg = r.TLegend(0.48, 0.25, 0.92, 0.5)
cfgs = ["single","one","all","none"]
names = ["p(#geq1 PE in given slab)","p(#geq1 PE in at least 1 slab)","p(#geq1 PE in all slabs)","p(0 PE in all slabs)"]
...
c.SaveAs(os.path.join(outdir, "nPE", "slab_nPE_probs.png"))
c.SaveAs(os.path.join(outdir, "nPE", "slab_nPE_probs.pdf"))
