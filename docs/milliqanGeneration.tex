...
\noindent {\bf Status of this: this was done by Itay, it has 
now been reproduced by Nick and Bennet, needs to be documented.}
...
\begin{figure}
\includegraphics[width=0.58\linewidth]{plots/fraction-from-b-theory.png}
\includegraphics[width=0.38\linewidth, angle=90]{plots/bFraction-CMS-314nb-CDF.pdf}
...
\begin{figure}
  ...
  \caption{\protect Cross-section times branching ratios for various
    decays into $\zeta^+ \zeta^-$ pairs, excluding DY.  Note: this is
    ...
    Also: the mother particle must be in $|\eta|<2$, but the
    daughter $\zeta^{\pm}$ could have $|\eta|>2$.  On the other hand some
    $\zeta$'s with $|\eta|<2$ can arise from mothers of $|\eta|>2$.}
